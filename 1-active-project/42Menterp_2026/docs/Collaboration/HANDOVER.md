...
- **변경 사항:**
    ...
    - `app/src/app/api/tax-invoices/export-hometax/route.ts` - 홈택스 양식 Excel Export
      - 정산 데이터 고객별 그룹핑
      - 공급가액/세액 자동 계산 (10%)
      - XLS/CSV 형식 지원
      - 요약 시트 포함
      - 스킵된 건 사유별 분류 (이메일 미등록, 사업자번호 미등록 등)

    #### T-02: 정산-세금계산서 연동
    - `app/src/app/api/tax-invoices/link-settlements/route.ts` - 정산 연동 API
      - POST: 정산을 세금계산서에 연결
      - DELETE: 정산 연결 해제
      - 세금계산서 금액 자동 합산 업데이트

    #### T-03: 발행 전 검증
    - `app/src/app/api/tax-invoices/pre-export-check/route.ts` - 발행 전 검증 API
      - 필수 검증: 사업자번호, 상호명, 이메일, 정산 금액
      - 권장 검증: 대표자명, 사업장 주소
      - 발행 불가(blockers) / 경고(warnings) 분류
      - 통계 요약 (발행 가능/불가 건수, 총 금액 등)
...
### 구현 범위 (Phase 1~5)
- **공용 컴포넌트**: DataTable, BulkActionBar, Excel Import/Export, Hooks
- **상품 마스터**: 20개 상품 (트래픽/길찾기/블로그/리뷰/영수증)
- **거래처 관리**: Import/Export API, 레거시 마이그레이션, 자동 연결
- **주간 발주**: Smart Grid UI, 저장 API, Manual Override 보호
- **고객/상품 고도화**: DataTable 리팩토링, 유형별 그룹화 뷰, 세금계산서 상태
- **세금계산서 자동화**: 홈택스 Export, 정산 연동, 발행 전 검증

### 주요 파일 (총 40+ 파일 생성/수정)
- `app/src/components/common/` - 공용 컴포넌트 (15개 파일)
- `app/src/app/api/` - API 라우트 (12개 파일)
- `app/src/app/(dashboard)/` - 페이지 (5개 파일)
- `app/prisma/` - 스키마 및 시드 (3개 파일)
