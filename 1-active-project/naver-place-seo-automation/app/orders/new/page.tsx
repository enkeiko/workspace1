...
export default function NewOrderPage() {
  ...
  useEffect(() => {
    if (formData.customerId) {
      ...
      setStores([]);
      setQuotations([]);
      setFormData((prev) => ({ ...prev, storeId: "", quotationId: "" }));
    }
  }, [formData.customerId]);
  ...
  const fetchQuotations = async (customerId: number) => {
    try {
      const response = await fetch(`/api/quotations?customerId=${customerId}&status=accepted&limit=100`);
      const result = await response.json();
  ...
  const loadQuotationItems = async (quotationId: number) => {
    try {
    ...
    try {
      ...
      const response = await fetch("/api/orders", {
        ...
        body: JSON.stringify({
          ...
          quotationId: formData.quotationId ? Number(formData.quotationId) : undefined,
      ...
      if (result.success) {
        toast.success("주문 생성 완료", {
          description: `${result.data.orderNumber} 주문이 성공적으로 생성되었습니다.`,
        });
        router.push(`/orders/${result.data.id}`);
      } else {
        toast.error("주문 생성 실패", {
          description: result.error?.message || "주문 생성에 실패했습니다.",
        });
      }
    } catch (error) {
    ...
    <MainLayout>
      <div className="container mx-auto p-6">
        <div className="mb-6 flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold">주문 추가</h1>
            <p className="text-gray-500">새 주문을 작성합니다.</p>
          </div>
        </div>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setFormData({ ...formData, customerId: value, storeId: "", quotationId: "" })
                    }
                ...
                {quotations.length > 0 && (
                  <div className="space-y-2">
                    <Label htmlFor="quotationId">견적서 (선택)</Label>
                    <Select
                      value={formData.quotationId || "none"}
                      onValueChange={(value) => setFormData({ ...formData, quotationId: value === "none" ? "" : value })}
                    >
                      <SelectTrigger id="quotationId">
                        <SelectValue placeholder="견적서를 선택하면 항목이 자동으로 채워집니다" />
                      </SelectTrigger>
                      <SelectContent>
